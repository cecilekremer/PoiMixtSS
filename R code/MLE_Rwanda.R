...
######################################################################################################
### DATA EXAMPLE 3: RWANDA (personal communication                                                 ###
######################################################################################################
...
## Observed offspring distribution
nsec = c(rep(0,643),rep(1,118),rep(2,23),rep(3,5),rep(4,4),5,6)
hist(nsec,breaks=6)
fitdist(nsec,"nbinom")
...
sample = rmvnorm(100000, mean=c(a.est,p.est,d.est), sigma=covmat)
R.est = c(); sd.est = c()
for(i in 1:dim(sample)[1]){
...
# jpeg("propRwanda.jpeg", width = 18, height = 15, units = 'cm', res = 1200)
plot(a,prop.NB,type="l",col="blue",main="(e)", xlab="Proportion of infectious cases",ylab="Expected proportion of transmission")
prop.LN <-c()
...
# p80%
abline(h=0.8,lty=2)
abline(v=a[324],lty=3,col="blue")
abline(v=a[389],lty=3,col="purple")
abline(v=a[331],lty=3,col="orange")
abline(v=a[344],lty=3,col="red")
# dev.off()
...
## Figure 2f
# jpeg("propRwanda3.jpeg", width = 18, height = 15, units = 'cm', res = 1200)
plot(prop.NB$x1,prop.NB$Prop,type="n",pch=15,col="dimgray", main="(f)",xlab="Proportion of infectious cases",ylab="Realized proportion of transmission",ylim=c(0,1),xlim=c(0,max.p+0.05))
...
legend(x=0.17,y=0.3,c("NB","POLN","POWB","POGG"),lty=c(1,1,1,1),col=c("blue","purple","orange","red"))
# dev.off()
...
text(x=0.1,y=0.7,labels=c(paste0("r = ",round(cor(observed,predicted.NB),5))),cex=0.7)
...
text(x=0.1,y=0.7,labels=c(paste0("r = ",round(cor(observed,predicted.LN),5))),cex=0.7)
...
text(x=0.1,y=0.7,labels=c(paste0("r = ",round(cor(observed,predicted.WB),5))),cex=0.7)
...
text(x=0.1,y=0.7,labels=c(paste0("r = ",round(cor(observed,predicted.GG),5))),cex=0.7)
...
n.sec = nsec
# jpeg("fitPareto3.jpeg", width = 22, height = 12, units = 'cm', res = 1200)
par(mfrow=c(1,2))
plot(unique(sort(n.sec)),as.numeric(table(n.sec))/length(n.sec), xlim=c(0,max(n.sec)), ylim=c(0,0.85), pch=17, 
